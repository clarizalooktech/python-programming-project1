...
#Task:
# Forensic accounting is about trying to detect the difference between normally occurring numbers, 
# and numbers that have been changed in order to falsify an account. Due to the natural randomness 
# in such accounts, it cannot be determined which individual numbers are fraudulent and which are 
# legitimate. However, the digits over many numbers can be examined to build up a large distribution.
# If this distribution looks significantly different to the one predicted by Benford’s Law, there is 
# some evidence that the account is fraudulent.
# The task for this project is to write a program that can calculate the distribution of digits for 
# numbers found in a certain file. You are not asked to actually detect any fraudulence.
#
#Notes from Clariza: 
# This program calculates the frequency distribution of a number up to 10 digits only.
#  .......During data processing stage, it skips the entire cell in the csv file if it finds 
#  .......characters that are letters, empty cells or non alpha numeric in that certain cell
#  .......It checks if file is not found, if no_place is = 0 or greater than 10
#Sample input: output = main("sample_accounts.csv",1)
#Sample output: [[26, 22, 28, 22, 16, 20, 31, 22, 13, 0]]
